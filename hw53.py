...
# ------ SETS ---------
model.d = pe.Set(initialize = range(2))
model.dd = pe.Set(initialize = range(3))
model.ddd = pe.Set(initialize = range(4))
model.le = pe.Set(initialize = range(30))
# ------PARAMETERS--------
model.Y = pe.Param(model.le, model.d, initialize = lambda model, i, j: y[i][j])
...
def precedence_rule(model, i):
    return model.p[i] >= 1 - model.Z[i]*(model.w[0] * model.Y[i,0] + model.w[1] * model.Y[i, 1]\
                            + model.w[2] * model.Y[i, 0] * model.Y[i, 0] - model.w[3])
...
def resource2_rule(model, i):
    return model.z[i] >= -model.w[i]
model.resource2Cons = pe.Constraint(model.dd, rule = resource2_rule)
# ------OBJECTIVE-----------
def obj_rule(model):
    temp = sum(model.p[i] for i in model.p)
    return temp + lamb * (model.z[0] + model.z[1] + model.z[2])
...
lineX = np.arange(0, max(y[:,0]), 0.1)
lineY = (model.w[3].value - lineX * model.w[0].value - lineX * lineX * model.w[2].value) / model.w[1].value
