...
model.worktime = pe.Param(model.worktime, 
                    initialize = lambda model, t: worktime[t])
...
'''If relax the two-third rule, domain need to change to
    pe.NonNegativeReals, or it will solve the problem very slow.'''
# ------CONSTRAINTS-----------
...
def twothirds_rule(model, c):
    ...
    for j in range(4):
        if c - j < 0:
            ...
            partres = partres + model.part[c-j]    
    return (fullres >= 2 * partres)
    '''Here, if we need to relax the two-thirds rule, we just need
       to change the last sentence to:
...
def obj_rule(model):
    fulltimewage = model.worktime[0] * model.wage[0] \
                    * sum(model.full[i] for i in model.times)
    parttimewage = model.worktime[1] * model.wage[1] \
                    * sum(model.part[i] for i in model.times)
...
for c in model.times:
    ...
    surplus = fullres + 5/6 * partres - model.ftee[c]
    print('At %d o\'clock %d full-time employee and %d part-time \
        employee currently working'%(c, fullres, partres))
    print('The surplus at %d o\'clock is %.2f value.'%(c, surplus)) 
